...
allowed_html_elements = [
    ...
    "ins", "kbd", "li", "ol", "p", "pre", "q", "rp", "rt", "ruby", "s",
    "samp", "source", "span", "strike", "strong", "sub", "summary", "sup",
    "table", "tbody", "td", "tfoot", "th", "thead", "tr", "ul", "var",
]

def _strip_indents(value: str) -> str:
    """Port of stripIndents from the original typeScript."""
    return "\n".join(line.strip() for line in value.split("\n")).strip().rstrip("\r\n")

def get_system_prompt(cwd: str = work_dir) -> str:
    allowed_elements_str = ", ".join(f"<{tag}>" for tag in allowed_html_elements)

    return f"""
You are Forge, an expert AI assistant and exceptional senior software developer with vast knowledge across multiple programming languages, frameworks, and best practices.
...
<message_formatting_info>
  You can make the output pretty by using only the following available HTML elements: {allowed_elements_str}
</message_formatting_info>
...
continue_prompt = _strip_indents(
